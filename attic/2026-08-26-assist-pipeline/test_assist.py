...
from b601.assist import DynamicAssist, TorqueRebalance  # noqa: E402
...
def test_dyn_assist_no_self_drive():
    """--dyn-assist observer: with no hand it must not move the arm (sensor weights
    vanish at rest; tau_sent self-accounting cancels its own injection)."""
    arm, ctrl = run_drag(Q_WRIST, assist=DynamicAssist(DYN))
    drift = np.abs(ctrl.q_drag_end - Q_WRIST)
    assert np.all(drift < 0.05), f"dyn assist alone moved the arm: {np.round(drift, 3)}"


def test_dyn_assist_recruits_base():
    """--dyn-assist observer: a lateral push must recruit j1, a radial push must recruit
    the shoulder and truly translate the EE farther, all without tripping vel_abort
    (the wrist is sensed but not driven - wrist_gain 0 keeps the loop stable)."""
    q0 = Q_WRIST
    lever1 = abs((DYN.ee_jacobian(q0).T @ [0, 1, 0, 0, 0, 0])[0])
    arm, ctrl = run_drag(q0, external=push_at_ee([0.0, 0.85 * COULOMB[0] / lever1, 0.0]),
                         assist=DynamicAssist(DYN))
    assert abs(ctrl.q_drag_end[0] - q0[0]) > 0.05, \
        f"lateral push must recruit j1, moved {abs(ctrl.q_drag_end[0] - q0[0]):.3f} rad"

    lev = DYN.ee_jacobian(q0)[:3].T @ [1.0, 0.0, 0.0]
    Fr = [1.8 * COULOMB[3] / abs(lev[3]), 0.0, 0.0]
    arm_p, ctrl_p = run_drag(q0, external=push_at_ee(Fr))
    arm_a, ctrl_a = run_drag(q0, external=push_at_ee(Fr), assist=DynamicAssist(DYN))
    dsh = np.abs(ctrl_a.q_drag_end[1:3] - q0[1:3]).max()
    dx_p = _ee_p(ctrl_p.q_drag_end)[0] - _ee_p(q0)[0]
    dx_a = _ee_p(ctrl_a.q_drag_end)[0] - _ee_p(q0)[0]
    assert dsh > 0.05, f"radial push must recruit the shoulder, moved {dsh:.3f} rad"
    assert dx_a > dx_p + 0.02, f"EE must translate farther: plain {dx_p:.3f} m, dyn {dx_a:.3f} m"
