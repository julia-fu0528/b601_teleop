...
def cmd_drag(cfg, dyn, args) -> None:
    ...
    lateral = None
    if args.lateral:
        from b601.assist import LateralAssist
        if not 0.0 < args.lateral <= 2.0:
            raise SystemExit("--lateral: joint1 torque cap in (0, 2] N.m (config tau_max clamp is 3)")
        lateral = LateralAssist(dyn, tau_max=args.lateral)
        print(f"lateral assist: joint1 servos to carry the sideways gripper motion the wrist yields, cap {args.lateral} N.m")

    ctrl = GravityDragController(
        arm, dyn, cfg, lateral=lateral,
...
def main() -> None:
    ...
    d.add_argument("--lateral", type=float, metavar="TAU",
                   help="sideways-push takeover: servo joint1 to carry the lateral gripper motion the wrist is "
                        "yielding; TAU = torque cap (N.m), above j1 breakaway on purpose, try 1.2")
